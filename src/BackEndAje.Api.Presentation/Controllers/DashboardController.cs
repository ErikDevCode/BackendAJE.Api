...
namespace BackEndAje.Api.Presentation.Controllers
{
    ...
    public class DashboardController : ControllerBase
    {
        ...
        [ProducesResponseType((int)HttpStatusCode.NotFound)]
        [Route("census/asset")]
        public async Task<IActionResult> GetCensusOrderRequestByFilters([FromQuery]int? regionId, [FromQuery]int? zoneId, [FromQuery]int? route, [FromQuery]int? month, [FromQuery]int? year)
        {
            var query = new GetCensusOrderRequestByFiltersQuery(regionId, zoneId, route, month, year);
            var clients = await this._mediator.Send(query);
            return this.Ok(new Response { Result = clients });
        }
    }
