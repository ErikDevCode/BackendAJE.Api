namespace BackEndAje.Api.Application.Asset.Queries.GetClientAssets
{
    ...
    public class GetClientAssetsHandler : IRequestHandler<GetClientAssetsQuery, PaginatedResult<GetClientAssetsResult>>
    {
        ...
        {
            var clientAssets = await this._clientAssetRepository.GetClientAssetsAsync(request.PageNumber, request.PageSize, request.CodeAje, request.ClientId, request.userId, request.CediId, request.RegionId, request.Route, request.ClientCode);
            var result = this._mapper.Map<List<GetClientAssetsResult>>(clientAssets);

            var totalAssets = await this._clientAssetRepository.GetTotalClientAssets(request.CodeAje, request.ClientId, request.userId, request.CediId, request.RegionId, request.Route, request.ClientCode);
